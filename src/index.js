...
// "news.ycombinator.com" → "ycombinator"; handles ccTLDs like .co.uk correctly
export function getBaseLabel(domain) {
  const parts = domain.split('.');
  let label;
  if (parts.length <= 2) {
    label = parts[0];              // "example.com" → "example"
  } else if (parts.length === 3) {
    label = parts[1];              // "sub.example.com" → "example"
  } else {
    label = parts[parts.length - 3]; // "sub.example.co.uk" → "example"
  }
...
async function handleCreateAlias(req, res) {
  ...
  const [localPart, domainPart] = created.email.split('@');
